...
关于 `fetch` 方法的更多信息，请参考 [页面 fetch 方法API](https://www.nuxtjs.cn/api/pages-fetch)。
...
这时[context](https://www.nuxtjs.cn/api/context)被赋予`nuxtServerInit`作为第二个参数，它与`asyncData`或`fetch`方法相同。
